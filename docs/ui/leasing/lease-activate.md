...
### Purpose
Review and activate an approved Lease while freezing terms and revalidating capacity.
...
### Breadcrumb
`Leasing / Lease Activate`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Lease Activate]
