...
### Purpose
Search, filter, compare, and act on Leases at Organization and authorized Property scope.
...
**API alignment:** `GET/POST /leases`. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Breadcrumb
`Leasing / Leases List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Leases List]
...
### Columns
Primary data table columns: Lease number, Residents, Rentable Item, dates, contract status, physical occupancy, balance, next action, and updated time.
