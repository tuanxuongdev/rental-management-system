...
### Purpose
Support the complete lease renew transfer task with clear context, guarded actions, and durable outcomes.
...
### Breadcrumb
`Leasing / Lease Renew Transfer`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Lease Renew Transfer]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and filter controls: Property; Unit or optional Bed; Leaseholders; occupants; dates; liability model; rent/currency; deposit; billing cadence; checklist evidence.
