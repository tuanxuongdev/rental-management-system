...
### Purpose
Coordinate surrender, inspection, final Meter readings, asset returns, deposit disposition, and final account handoff.

### Target users
Leasing Officer, Property Manager, Organization administrators, and read-only Finance/Audit roles.
...
**API alignment:** Lease lifecycle, allocation, move event, and deposit endpoints in API §13. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
**Authorization:** `leases.list/view/create/update/approve/activate/renew/terminate` and lease document permissions; workflow-specific step-up and approvals apply. UI checks shape discoverability only; the server remains authoritative.
...
### Breadcrumb
`Leasing / Move Out Checkout`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.

### Sidebar
Highlight Leasing using icon, label, and left indicator. Hide unavailable domains; show disabled conditional destinations only when an explanation helps users request access or complete setup.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Move Out Checkout]
...
### Field list
The primary experience is a form/workflow. Fields and filter controls: Property; Unit or optional Bed; Leaseholders; occupants; dates; liability model; rent/currency; deposit; billing cadence; checklist evidence.
...
### Columns
Related-record tables use: Lease number, Residents, Rentable Item, dates, contract status, physical occupancy, balance, next action, and updated time. The primary record itself uses a description list, not a one-row table.
