...
### Purpose
Support the complete mfa challenge task with clear context, guarded actions, and durable outcomes.
...
**API alignment:** `POST /auth/mfa/challenges` and `/auth/mfa/challenges/{challengeId}/verify`. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and controls: Allowed method selector (TOTP, WebAuthn/passkey, or recovery code); proof input appropriate to method; trust-device option only when policy permits.
