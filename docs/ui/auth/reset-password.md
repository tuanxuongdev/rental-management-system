...
### Purpose
Support the complete reset password task with clear context, guarded actions, and durable outcomes.
...
**API alignment:** `POST /auth/password/reset`. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and controls: Single-use token from the link (not displayed); new password (required, 12–128+ characters); confirm password; revoke all sessions defaults to on.
