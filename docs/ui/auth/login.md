...
### Purpose
Authenticate securely, choose an eligible Organization, and begin a workforce or Resident session without account enumeration.
...
**API alignment:** `/auth/*`, `/me`, and invitation acceptance endpoints in API §9–10. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and controls: Email (required, normalized); password (required); Organization selector only after eligibility is safely established; remember-me preference; device name derived but editable.
