...
### Purpose
Support the complete invitation accept task with clear context, guarded actions, and durable outcomes.
...
**API alignment:** `POST /invitations/{invitationId}/accept`. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and controls: Invitation token (not displayed); Organization and proposed access review; authenticated account switch when email differs; display name/password only for an approved new-account path; explicit acceptance.
