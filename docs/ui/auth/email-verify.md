...
### Purpose
Support the complete email verify task with clear context, guarded actions, and durable outcomes.
...
**API alignment:** `POST /auth/email/verify` and resend. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and controls: Single-use verification token from the link (not displayed); resend email only when verification cannot complete.
