...
### Purpose
Find authorized Properties, Units, Residents, Leases, Payments, and maintenance records from one keyboard-first surface.
...
### Breadcrumb
`Shell / Global Search`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Global Search]
