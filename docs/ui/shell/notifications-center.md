...
### Purpose
Search, filter, compare, and act on Notifications Center at Organization and authorized Property scope.
...
### Breadcrumb
`Shell / Notifications Center`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Notifications Center]
