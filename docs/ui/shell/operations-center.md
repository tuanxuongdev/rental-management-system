...
### Purpose
Track durable asynchronous billing, imports, exports, reports, document scans, and bulk actions after users navigate away.
...
### Breadcrumb
`Shell / Operations Center`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Operations Center]
...
### Columns
Primary data table columns: Entity/result, contextual identifier, Property, status, timestamp, and permitted quick action.
