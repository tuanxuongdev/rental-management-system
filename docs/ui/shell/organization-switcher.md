...
### Purpose
Change the active Organization using token exchange while preventing any prior-Organization state from remaining visible.

### Target users
All authenticated staff; results and controls are reduced to effective Organization, Property, role, and data-class scope.
...
**API alignment:** `/me`, `/organizations`, token exchange, `/notifications`, `/operations`, and authorized domain searches. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
**Authorization:** Effective permissions from `/me`; shell visibility is presentation only and never substitutes for API authorization. UI checks shape discoverability only; the server remains authoritative.
...
### Breadcrumb
`Shell / Organization Switcher`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.

### Sidebar
Highlight Shell using icon, label, and left indicator. Hide unavailable domains; show disabled conditional destinations only when an explanation helps users request access or complete setup.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Organization Switcher]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and filter controls: Search query; entity filters; Organization; Property scope; notification status; operation status.
...
### Columns
Related-record tables use: Entity/result, contextual identifier, Property, status, timestamp, and permitted quick action. The primary record itself uses a description list, not a one-row table.
