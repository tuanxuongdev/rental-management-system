...
### Purpose
Monitor service health, Organization lifecycle exceptions, security alerts, usage pressure, and support-access activity without exposing unnecessary Resident-level data.
...
### Breadcrumb
`Platform / Platform Dashboard`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Platform Dashboard]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and filter controls: Organization reference; lifecycle state; case/reason; requested scope; duration; approver; service/health filters.
