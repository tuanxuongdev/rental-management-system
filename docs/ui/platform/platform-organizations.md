...
### Purpose
Find and administer Organization provisioning, subscription/lifecycle state, placement, and safe service metadata without implicit access to business records.
...
### Breadcrumb
`Platform / Platform Organizations`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Platform Organizations]
