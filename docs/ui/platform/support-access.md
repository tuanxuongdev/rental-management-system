...
### Purpose
Request, approve, enter, and end time-bound support access without silent impersonation.

### Target users
Platform Administrator and Support Agent with mandatory MFA.

### Entry points
- Platform-only sidebar after mandatory MFA and platform-audience authentication.
- Service/security alerts, Organization lifecycle exceptions, support case links, and active support-access banner.
- Organization business records are not deep-linked until an approved support-access grant is active.

**API alignment:** Platform service endpoints plus controlled support-access resources; Organization business APIs are used only inside an approved elevated context. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
**Authorization:** `platform.*` and `support.elevate`; no implicit Organization-data access. Support access is approved, time-bound, visibly marked, and dual-identity audited. UI checks shape discoverability only; the server remains authoritative.
...
### Breadcrumb
`Platform / Support Access`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.

### Sidebar
Highlight Platform using icon, label, and left indicator. Hide unavailable domains; show disabled conditional destinations only when an explanation helps users request access or complete setup.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Support Access]
...
### Field list
The primary experience is a form/workflow. Fields and filter controls: Organization reference; lifecycle state; case/reason; requested scope; duration; approver; service/health filters.
...
### Columns
Related-record tables use: Organization/service, region/cell, subscription/lifecycle, health, active support access, owner, updated time, and platform action. The primary record itself uses a description list, not a one-row table.
...
### Primary
**Save draft / Continue / Complete the named workflow.** Use one emphasized button with a specific verb and disabled/loading text that communicates progress.
