...
### Purpose
Show the Resident’s current balance, next due item, Lease dates, unread notifications, active maintenance requests, and recent documents with self-safe drill-downs.
...
### Breadcrumb
`Resident portal / Home` on desktop; mobile uses page title plus Back only for nested detail.
...
```mermaid
flowchart LR
  A[Authenticated Resident] --> B[Home]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and controls: Optional Lease selector when the Resident has multiple self-linked Leases; no editable business fields. Cards expose balance currency, next due date, Lease dates, maintenance state, unread count, and freshness.
