...
### Purpose
Review and update self-editable profile, contacts, communication preferences, sessions, password, and MFA while keeping legal identity fields controlled.
...
### Breadcrumb
`Resident portal / Profile` on desktop; mobile uses page title plus Back only for nested detail.
...
```mermaid
flowchart LR
  A[Authenticated Resident] --> B[Profile]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and controls: Preferred name, allowed contacts, emergency contacts, locale, communication preferences, quiet hours/time zone, password/MFA/session controls; legal identity is read-only.
