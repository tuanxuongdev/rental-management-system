...
### Purpose
Create and track maintenance requests for self-linked occupancy while exposing only resident-safe status, messages, appointments, and documents.
...
### Breadcrumb
`Resident portal / Maintenance` on desktop; mobile uses page title plus Back only for nested detail.
...
```mermaid
flowchart LR
  A[Authenticated Resident] --> B[Maintenance]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and controls: For create: Lease, server-validated Unit, category, description, priority, access availability, contact preference, and READY attachments. For tracking: status/Lease/date filters.
