...
### Purpose
List and explain resident-visible invoices, line items, credits, allocations, due dates, currency, and outstanding amounts for self-linked obligations.
...
### Breadcrumb
`Resident portal / Invoices` on desktop; mobile uses page title plus Back only for nested detail.
...
```mermaid
flowchart LR
  A[Authenticated Resident] --> B[Invoices]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and controls: Lease, invoice status, due-before, and has-balance filters; payment amount/invoice selection appears only when starting a Payment.
