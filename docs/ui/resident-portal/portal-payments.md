...
### Purpose
Initiate an eligible Payment and review self-visible Payment status, allocation, receipts, refunds, and provider handoff without implying settlement prematurely.
...
### Breadcrumb
`Resident portal / Payments` on desktop; mobile uses page title plus Back only for nested detail.
...
```mermaid
flowchart LR
  A[Authenticated Resident] --> B[Payments]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and controls: Lease/date/status filters; for initiation: selected self-visible obligations, amount, currency, enabled channel, fee disclosure acknowledgement, and provider-required next step.
