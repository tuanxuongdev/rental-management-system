...
### Purpose
Find, preview, and download explicitly resident-visible Lease, invoice, receipt, notice, and maintenance documents through short-lived authorized access.
...
### Breadcrumb
`Resident portal / Documents` on desktop; mobile uses page title plus Back only for nested detail.
...
```mermaid
flowchart LR
  A[Authenticated Resident] --> B[Documents]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and controls: Lease, category, status, date, and title search; no visibility or retention editing.
