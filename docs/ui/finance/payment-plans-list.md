...
### Purpose
Search, filter, compare, and act on Payment Plans at Organization and authorized Property scope.
...
### Breadcrumb
`Finance / Payment Plans List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Payment Plans List]
