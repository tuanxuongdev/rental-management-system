...
### Purpose
Search, filter, compare, and act on Expenses at Organization and authorized Property scope.
...
### Breadcrumb
`Finance / Expenses List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Expenses List]
