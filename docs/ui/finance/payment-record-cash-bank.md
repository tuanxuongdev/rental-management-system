...
### Purpose
Record cash, bank transfer, QR, or other external Payments with evidence, allocation preview, and settlement distinction.

### Target users
Accountant/Finance Officer, Organization Owner, authorized Property Manager, approvers, and Read-only Auditor.
...
**API alignment:** Meter/utility, invoice, payment, expense, and reconciliation endpoints in API §15–18. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
**Authorization:** `finance.*`, `meters.*`, `utilities.*`, `arrears.*`, and expense permissions. Refunds, write-offs, reconciliation, and deposit release enforce separation of duties. UI checks shape discoverability only; the server remains authoritative.
...
### Breadcrumb
`Finance / Payment Record Cash Bank`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.

### Sidebar
Highlight Finance using icon, label, and left indicator. Hide unavailable domains; show disabled conditional destinations only when an explanation helps users request access or complete setup.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Payment Record Cash Bank]
...
### Field list
The primary experience is a form/workflow. Fields and filter controls: Property; Resident/Lease; currency; amount as decimal; transaction/accounting dates; reference; status; evidence; reason; approval data.
...
### Columns
Related-record tables use: Stable number, Resident/Lease, Property, date, amount with currency, status, allocation/reconciliation state, owner, and actions. The primary record itself uses a description list, not a one-row table.
