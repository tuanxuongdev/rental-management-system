...
### Purpose
Support the complete deposit disposition task with clear context, guarded actions, and durable outcomes.
...
### Breadcrumb
`Finance / Deposit Disposition`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Deposit Disposition]
