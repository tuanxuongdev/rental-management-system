...
### Purpose
Prioritize overdue accounts, explain eligibility, coordinate collection actions, and respect disputes and payment plans.
...
### Breadcrumb
`Finance / Arrears Workspace`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Arrears Workspace]
