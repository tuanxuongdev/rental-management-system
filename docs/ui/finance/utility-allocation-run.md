...
### Purpose
Preview and post reproducible shared-utility allocations with transparent inputs, rounding, and reconciliation.
...
### Breadcrumb
`Finance / Utility Allocation Run`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Utility Allocation Run]
