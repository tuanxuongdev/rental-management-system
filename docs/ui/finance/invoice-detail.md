...
### Purpose
Provide a complete, traceable view of one Invoice and its permitted related records and actions.
...
**API alignment:** `GET/PATCH /invoices/{invoiceId}` plus lines, post, send, void, allocations, and credit notes. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Breadcrumb
`Finance / Invoice Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Invoice Detail]
