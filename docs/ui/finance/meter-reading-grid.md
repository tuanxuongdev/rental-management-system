...
### Purpose
Enter or paste high-volume readings for Meters with meterType ELECTRICITY or WATER using row-level validation.
...
### Breadcrumb
`Finance / Meter Reading Grid`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Meter Reading Grid]
