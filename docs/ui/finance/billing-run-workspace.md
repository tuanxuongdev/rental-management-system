...
### Purpose
Preview, validate, approve, execute, and safely retry recurring billing with a durable run record.
...
### Breadcrumb
`Finance / Billing Run Workspace`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Billing Run Workspace]
...
### Columns
Primary data table columns: Stable number, Resident/Lease, Property, date, amount with currency, status, allocation/reconciliation state, owner, and actions.
