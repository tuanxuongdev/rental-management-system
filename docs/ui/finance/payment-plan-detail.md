...
### Purpose
Provide a complete, traceable view of one Payment Plan and its permitted related records and actions.
...
### Breadcrumb
`Finance / Payment Plan Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Payment Plan Detail]
