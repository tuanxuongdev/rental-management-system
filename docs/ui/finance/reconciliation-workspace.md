...
### Purpose
Match Payments to bank/provider/cash-up evidence and resolve exceptions under separation of duties.
...
### Breadcrumb
`Finance / Reconciliation Workspace`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Reconciliation Workspace]
