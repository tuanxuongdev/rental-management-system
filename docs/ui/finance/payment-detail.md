...
### Purpose
Provide a complete, traceable view of one Payment and its permitted related records and actions.
...
**API alignment:** `GET /payment-transactions/{transactionId}` plus allocation, reversal, refund, receipt, and reconciliation links. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Breadcrumb
`Finance / Payment Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Payment Detail]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and filter controls: Property; Resident/Lease; currency; amount as decimal; transaction/accounting dates; reference; status; evidence; reason; approval data.
