...
### Purpose
Search, filter, compare, and act on Invoices at Organization and authorized Property scope.
...
**API alignment:** `GET/POST /invoices` and `/invoice-aging` where summarized. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Breadcrumb
`Finance / Invoices List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Invoices List]
