...
### Purpose
Provide a complete, traceable view of one Expense and its permitted related records and actions.
...
### Breadcrumb
`Finance / Expense Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Expense Detail]
