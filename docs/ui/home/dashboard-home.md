...
### Purpose
Present role-aware, scoped exceptions and decisions rather than decorative metrics.

### Target users
Organization Owner, Organization Administrator, Property Manager, Accountant, Maintenance Staff, and Read-only Auditor.
...
**API alignment:** `/dashboard/portfolio-summary`, `/dashboard/occupancy`, `/dashboard/finance`, and `/dashboard/operations`. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
**Authorization:** `reports.operational.view`, `reports.occupancy.view`, and/or `reports.financial.view`; widgets are omitted independently when unauthorized. UI checks shape discoverability only; the server remains authoritative.
...
### Breadcrumb
`Home / Dashboard Home`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.

### Sidebar
Highlight Home using icon, label, and left indicator. Hide unavailable domains; show disabled conditional destinations only when an explanation helps users request access or complete setup.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Dashboard Home]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and filter controls: Property scope; as-of date/time; comparison period; currency where financial totals are shown.
...
### Columns
Related-record tables use: Exception, affected resource, Property, severity, owner, due date, and status. The primary record itself uses a description list, not a one-row table.
