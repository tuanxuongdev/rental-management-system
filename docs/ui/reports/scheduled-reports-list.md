...
### Purpose
Search, filter, compare, and act on Scheduled Reports at Organization and authorized Property scope.
...
### Breadcrumb
`Reports / Scheduled Reports List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Scheduled Reports List]
...
### Columns
Primary data table columns: Report, scope, period/as-of, format, status, freshness, creator, created/completed time, and expiry.
