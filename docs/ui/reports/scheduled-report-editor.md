...
### Purpose
Create or update Scheduled Report with validation, impact preview, and safe concurrency handling.

### Target users
Executives, Organization Owner/Administrator, Property Manager, Accountant, and Read-only Auditor according to report definition.
...
**API alignment:** Report definition, run, download, and schedule endpoints in API §21. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
**Authorization:** Definition-specific `reports.*`; PII or bulk output additionally requires export permission, purpose, step-up, and full-scope revalidation. UI checks shape discoverability only; the server remains authoritative.
...
### Breadcrumb
`Reports / Scheduled Report Editor`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.

### Sidebar
Highlight Reports using icon, label, and left indicator. Hide unavailable domains; show disabled conditional destinations only when an explanation helps users request access or complete setup.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Scheduled Report Editor]
...
### Field list
The primary experience is a form/workflow. Fields and filter controls: Report definition; Property scope; as-of/period; time zone; currency; format; purpose; schedule; recipients; expiry.
...
### Columns
Related-record tables use: Report, scope, period/as-of, format, status, freshness, creator, created/completed time, and expiry. The primary record itself uses a description list, not a one-row table.
