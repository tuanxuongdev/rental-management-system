...
### Purpose
Search, filter, compare, and act on Work Orders at Organization and authorized Property scope.
...
### Breadcrumb
`Maintenance / Work Orders List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Work Orders List]
