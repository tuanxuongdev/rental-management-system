...
### Purpose
Provide a complete, traceable view of one Work Order and its permitted related records and actions.
...
### Breadcrumb
`Maintenance / Work Order Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Work Order Detail]
