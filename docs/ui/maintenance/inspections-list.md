...
### Purpose
Search, filter, compare, and act on Inspections at Organization and authorized Property scope.
...
### Breadcrumb
`Maintenance / Inspections List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Inspections List]
