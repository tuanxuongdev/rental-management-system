...
### Purpose
Search, filter, compare, and act on Maintenance Requests at Organization and authorized Property scope.
...
### Breadcrumb
`Maintenance / Maintenance Requests List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Maintenance Requests List]
...
### Columns
Primary data table columns: Reference, location, priority, status, assignee, SLA due, requester-safe identity, age, and action.
