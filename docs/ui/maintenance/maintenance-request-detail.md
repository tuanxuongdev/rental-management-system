...
### Purpose
Provide a complete, traceable view of one Maintenance Request and its permitted related records and actions.

### Target users
Maintenance Manager, assigned Maintenance Staff or Vendor, Property Manager, and Read-only Auditor.
...
**API alignment:** Maintenance and Work Order endpoints in API §19; inspection contracts follow the same scoped, versioned conventions. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
**Authorization:** `maintenance.requests.*`, `maintenance.work_orders.*`, `maintenance.costs.*`, and `inspections.*`; assigned workers see minimum necessary Resident information. UI checks shape discoverability only; the server remains authoritative.
...
### Breadcrumb
`Maintenance / Maintenance Request Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.

### Sidebar
Highlight Maintenance using icon, label, and left indicator. Hide unavailable domains; show disabled conditional destinations only when an explanation helps users request access or complete setup.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Maintenance Request Detail]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and filter controls: Property; Unit/Bed; category; description; priority; access window; assignee; schedule; SLA; checklist; costs; evidence.
...
### Columns
Related-record tables use: Reference, location, priority, status, assignee, SLA due, requester-safe identity, age, and action. The primary record itself uses a description list, not a one-row table.
