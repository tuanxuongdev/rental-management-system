...
### Purpose
Search, filter, compare, and act on Waitlist at Organization and authorized Property scope.
...
**API alignment:** `GET/POST /waitlist-entries` and entry update/remove actions. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Breadcrumb
`Residents / Waitlist`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
### Columns
Primary data table columns: Resident, verified contact, current Property/Unit, Lease status, portal status, balance indicator when allowed, and updated time.
