...
### Purpose
Provide a complete, traceable view of one Resident and its permitted related records and actions.
...
**API alignment:** `GET/PATCH /residents/{residentId}` plus contacts, history, identity documents, and portal routes. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Breadcrumb
`Residents / Resident Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Resident Detail]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and filter controls: Legal/preferred name; contacts; status; language; communication consent; emergency contact; portal state; waitlist criteria.
