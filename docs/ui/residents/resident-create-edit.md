...
### Purpose
Create or update Resident with validation, impact preview, and safe concurrency handling.

### Target users
Property Manager, Leasing Officer, authorized Organization administrators, and restricted Finance/Audit users.
...
**API alignment:** Resident and waitlist endpoints in API §12. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
**Authorization:** `residents.*`, `residents.sensitive_data.view`, and `waitlist.*`; sensitive fields and history are independently filtered. UI checks shape discoverability only; the server remains authoritative.
...
### Breadcrumb
`Residents / Resident Create Edit`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.

### Sidebar
Highlight Residents using icon, label, and left indicator. Hide unavailable domains; show disabled conditional destinations only when an explanation helps users request access or complete setup.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Resident Create Edit]
...
### Field list
The primary experience is a form/workflow. Fields and filter controls: Legal/preferred name; contacts; status; language; communication consent; emergency contact; portal state; waitlist criteria.
...
### Columns
Related-record tables use: Resident, verified contact, current Property/Unit, Lease status, portal status, balance indicator when allowed, and updated time. The primary record itself uses a description list, not a one-row table.
