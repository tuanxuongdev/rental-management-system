...
### Purpose
Provide a complete, traceable view of one Document and its permitted related records and actions.
...
**API alignment:** `GET /documents/{documentId}` plus versions, status, links, download, and retention-aware delete. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Breadcrumb
`Documents / Document Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Document Detail]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and filter controls: File; title; category; visibility; linked record; retention class; change reason; checksum and detected type are system supplied.
