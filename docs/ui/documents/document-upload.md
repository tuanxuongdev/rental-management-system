...
### Purpose
Upload a private document through constrained storage, scanning, classification, and explicit resource linking.

### Target users
Authorized staff and Residents through explicit self-visible links.
...
**API alignment:** Private upload, scan, version, link, download, and retention endpoints in API §23. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
**Authorization:** `documents.list/view/upload/delete` plus linked-resource permission; sensitive downloads and deletion are audited and may require step-up. UI checks shape discoverability only; the server remains authoritative.
...
### Breadcrumb
`Documents / Document Upload`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.

### Sidebar
Highlight Documents using icon, label, and left indicator. Hide unavailable domains; show disabled conditional destinations only when an explanation helps users request access or complete setup.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Document Upload]
...
### Field list
The primary experience is a form/workflow. Fields and filter controls: File; title; category; visibility; linked record; retention class; change reason; checksum and detected type are system supplied.
...
### Columns
Related-record tables use: Title, category, linked record, version, scan/status, visibility, owner, updated time, retention/hold indicator. The primary record itself uses a description list, not a one-row table.
