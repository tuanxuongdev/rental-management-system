...
### Purpose
Search, filter, compare, and act on Documents Library at Organization and authorized Property scope.
...
**API alignment:** `GET /documents`. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Breadcrumb
`Documents / Documents Library`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Documents Library]
...
### Columns
Primary data table columns: Title, category, linked record, version, scan/status, visibility, owner, updated time, retention/hold indicator.
