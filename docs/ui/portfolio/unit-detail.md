...
### Purpose
Provide a complete, traceable view of one Unit and its permitted related records and actions.
...
**API alignment:** `GET/PATCH /units/{unitId}` and status/amenity routes. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Breadcrumb
`Portfolio / Unit Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Unit Detail]
