...
### Purpose
Find advisory Unit, optional Bed, or capacity availability for a date range before a transactionally revalidated assignment.
...
### Breadcrumb
`Portfolio / Availability Lookup`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Availability Lookup]
