...
### Purpose
Search, filter, compare, and act on Beds at Organization and authorized Property scope.
...
**API alignment:** `GET/POST /units/{unitId}/beds` and `/beds/{bedId}`. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Breadcrumb
`Portfolio / Beds List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Beds List]
