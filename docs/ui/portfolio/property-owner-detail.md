...
### Purpose
Provide a complete, traceable view of one Property Owner and its permitted related records and actions.
...
### Breadcrumb
`Portfolio / Property Owner Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Property Owner Detail]
