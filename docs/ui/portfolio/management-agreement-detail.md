...
### Purpose
Provide a complete, traceable view of one Management Agreement and its permitted related records and actions.
...
### Breadcrumb
`Portfolio / Management Agreement Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Management Agreement Detail]
