...
### Purpose
Search, filter, compare, and act on Buildings at Organization and authorized Property scope.
...
### Breadcrumb
`Portfolio / Buildings List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Buildings List]
