...
### Purpose
Search, filter, compare, and act on Property Owners at Organization and authorized Property scope.
...
### Breadcrumb
`Portfolio / Property Owners List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Property Owners List]
