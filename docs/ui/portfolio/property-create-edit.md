...
### Purpose
Create or update Property with validation, impact preview, and safe concurrency handling.
...
### Breadcrumb
`Portfolio / Property Create Edit`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Property Create Edit]
...
### Field list
The primary experience is a form/workflow. Fields and filter controls: Code; name; Property; Building; Unit type; allocation mode; capacity; operational status; effective dates; owner/agreement attributes.
...
### Columns
Related-record tables use: Code/name, Property, type, capacity or share, occupancy/authority status, effective dates, updated time, and row actions. The primary record itself uses a description list, not a one-row table.
