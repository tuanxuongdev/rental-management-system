...
### Purpose
Provide a complete, traceable view of one Property and its permitted related records and actions.
...
**API alignment:** `GET/PATCH/DELETE /properties/{propertyId}` plus scoped summary includes. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
...
### Breadcrumb
`Portfolio / Property Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Property Detail]
