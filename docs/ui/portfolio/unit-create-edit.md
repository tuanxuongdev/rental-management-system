...
### Purpose
Create or update Unit with validation, impact preview, and safe concurrency handling.
...
### Breadcrumb
`Portfolio / Unit Create Edit`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Unit Create Edit]
