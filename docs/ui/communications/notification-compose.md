...
### Purpose
Resolve a permission-safe audience, preview localized content and exclusions, then schedule or send one logical notification.

### Target users
Organization Administrator, Property Manager, authorized Finance/Operations senders, and Auditors with view access.
...
**API alignment:** Notification, delivery, preference, and template endpoints in API §22. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
**Authorization:** `communications.notices.*` and `communications.templates.*`; audience expansion also requires permission to view each underlying data class. UI checks shape discoverability only; the server remains authoritative.
...
### Breadcrumb
`Communications / Notification Compose`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.

### Sidebar
Highlight Communications using icon, label, and left indicator. Hide unavailable domains; show disabled conditional destinations only when an explanation helps users request access or complete setup.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Notification Compose]
...
### Field list
The primary experience is a form/workflow. Fields and filter controls: Audience selector; channel; locale; classification; template/version; subject/body; variables; schedule; consent exclusions.
...
### Columns
Related-record tables use: Subject/reference, audience summary, channel, locale, schedule, delivery status/counts, creator, and created time. The primary record itself uses a description list, not a one-row table.
