...
### Purpose
Search, filter, compare, and act on Notification Templates at Organization and authorized Property scope.
...
### Breadcrumb
`Communications / Notification Templates List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Notification Templates List]
...
### Field list
The primary record is read-oriented; filters, inline notes, and permitted quick actions remain forms. Fields and filter controls: Audience selector; channel; locale; classification; template/version; subject/body; variables; schedule; consent exclusions.
...
### Columns
Primary data table columns: Subject/reference, audience summary, channel, locale, schedule, delivery status/counts, creator, and created time.
