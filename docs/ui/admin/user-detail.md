...
### Purpose
Provide a complete, traceable view of one User and its permitted related records and actions.
...
### Breadcrumb
`Admin / User Detail`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[User Detail]
