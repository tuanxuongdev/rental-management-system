...
### Purpose
Map, validate, review, and commit source rows with durable provenance, row errors, and idempotent operation tracking.

### Target users
Organization Owner and Organization Administrator; Security/Finance approvers and Auditors receive explicit limited access.
...
**API alignment:** Organization/access API §10 and import/export/audit/integration API §24–25. Collection pages use bounded cursor pagination (default 25, maximum 100); writes use `If-Match` and/or `Idempotency-Key` exactly where API §7 marks them. Errors map from RFC 9457 Problem Details and stable domain codes.  
**Authorization:** `organization.*`, `members.*`, `roles.*`, integration, import, export, and audit permissions; owner-only and high-risk operations require step-up. UI checks shape discoverability only; the server remains authoritative.
...
### Breadcrumb
`Admin / Import Wizard`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.

### Sidebar
Highlight Admin using icon, label, and left indicator. Hide unavailable domains; show disabled conditional destinations only when an explanation helps users request access or complete setup.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Import Wizard]
...
### Field list
The primary experience is a form/workflow. Fields and filter controls: Configuration-specific fields; reason and purpose for high-risk actions; effective dates; scope; role/permission set; provider-safe settings.
...
### Columns
Related-record tables use: Name/reference, type, scope, status, risk/permission summary, owner/actor, updated time, and permitted actions. The primary record itself uses a description list, not a one-row table.
