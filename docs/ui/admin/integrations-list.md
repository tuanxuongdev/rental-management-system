...
### Purpose
Search, filter, compare, and act on Integrations at Organization and authorized Property scope.
...
### Breadcrumb
`Admin / Integrations List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Integrations List]
...
### Columns
Primary data table columns: Name/reference, type, scope, status, risk/permission summary, owner/actor, updated time, and permitted actions.
