...
### Purpose
Create or update a delegable custom role with impact preview and forbidden-combination safeguards.
...
### Breadcrumb
`Admin / Role Editor`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Role Editor]
