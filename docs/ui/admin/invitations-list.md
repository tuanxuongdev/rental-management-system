...
### Purpose
Search, filter, compare, and act on Invitations at Organization and authorized Property scope.
...
### Breadcrumb
`Admin / Invitations List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Invitations List]
