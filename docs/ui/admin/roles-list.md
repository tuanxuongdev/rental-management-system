...
### Purpose
Search, filter, compare, and act on Roles at Organization and authorized Property scope.
...
### Breadcrumb
`Admin / Roles List`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Roles List]
