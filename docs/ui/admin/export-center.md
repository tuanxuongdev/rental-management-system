...
### Purpose
Request, monitor, and download authorized exports with purpose, expiry, step-up, and download audit.
...
### Breadcrumb
`Admin / Export Center`; detail/workflow screens insert the authorized parent label and record identifier. Breadcrumbs never contain sensitive values.
...
```mermaid
flowchart LR
  A[Authorized entry] --> B[Export Center]
